...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// To Do - your code here:
  	...
  	// Unnormalized probabilities will be stores in "total" 
	float total = 0.0;
	int height = grid.size();
	int width = grid[0].size();

	//Calculating the sum of the probability and storing it in total
	...
	newGrid = zeros(height, width); //Defining the grid space where the normalized probabilities will be

	//A loop to normalize the probabilities and store the resulting answers in newGrid
	for (int r = 0; r<height; r++)
	{
		for (int c = 0; c<width; c++)
		{
